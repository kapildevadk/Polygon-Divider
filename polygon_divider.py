...
from qgis.core import Qgis, QgsProject, QgsMessageLog, QgsGeometry, QgsFields, QgsField, QgsVectorFileWriter, QgsWkbTypes, QgsMapLayer
...
# ---------------------------------- MAIN FUNCTION ------------------------------------- #
    ...
    def work(self):
        ...
        * Actually do the processing
        ...
        if flds.lookupField('POLY_ID') == -1:
            flds.append(QgsField('POLY_ID',QVariant.Int))
        if flds.lookupField('UNIQUE_ID') == -1:
            flds.append(QgsField('UNIQUE_ID', QVariant.String))
        if flds.lookupField('AREA') == -1:
            flds.append(QgsField('AREA', QVariant.Double))
        if flds.lookupField('AREA') == -1:
...
#         fieldList.append(QgsField('ps_repPointY',QVariant.Int))
        ...
        # create a new shapefile to write the results to
            writer = QgsVectorFileWriter(outFilePath, "CP1250", flds, QgsWkbTypes.Polygon, layer.crs(), "ESRI Shapefile") 
        ...
        for feat in iter:
            geom = feat.geometry()
            if geom.type() == QgsWkbTypes.Polygon or feat.geometryType() == QgsWkbTypes.PolygonGeometry:    
...
def worker_error(e):
    QgsMessageLog.logMessage(u'Exception in worker thread: {}'.format(e))
...
class polygondivider:
    ...
    def run(self):
        ...
        * Run method that performs all the real work
        ...
        layers = [layer for layer in list(QgsProject.instance().mapLayers().values())]
        layer_list = []
        for layer in layers:
