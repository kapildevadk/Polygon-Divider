...
from qgis.core import Qgis,QgsFeature,QgsProject, QgsMessageLog, QgsGeometry, QgsPointXY, QgsFields, QgsField, QgsVectorFileWriter, QgsWkbTypes, QgsMapLayer, QgsVectorLayer
...
# ---------------------------------- MAIN FUNCTION ------------------------------------- #
    ...
    def work(self):
        ...
        * Actually do the processing
        ...
        flds.append(QgsField('POINTX',QVariant.Int))
        flds.append(QgsField('POINTY',QVariant.Int))
                        ...
                        if (interval[1]-interval[0]) > sq: 
                            ...
                            if ERROR_FLAG_1:
                                ...
                                # if none of the above worked, just skip it and move to a new feature
                                    ...
                                    if int((j*1) // totalDivisions * 100.0) > currProgress:
                                           currProgress = int((j*1) // totalDivisions * 100.0)
                                           self.progress.emit(currProgress)
                                        #self.progress.emit(j // totalDivisions * 100.0)
        ...
        if self.killed:
               ...
               # writer.rollback()
               # QgsProject.instance().removeMapLayer(writer)
            self.cleanup()
            raise UserAbortedNotification('USER Killed')
