...
#         fieldList.append(QgsField('ps_repPointY',QVariant.Int))
        ...
        #                                            forward_flag = not forward_flag
                ...
                QgsMessageLog.logMessage("Whoops! That dataset isn't polygons!", level=Qgs.CRITICAL)
...
def start_worker(self, worker, iface, message, with_progress=True):
    ...
    * Launch the worker thread
    ...
    worker.finished.connect(lambda result: worker_finished(
        result, thread, worker, iface, progressMessageBar))
    ...
    thread.start()
    return thread, progressMessageBar


def worker_finished(result, thread, worker, iface, progressMessageBar):
    """
    * Open the resulting file and clean up after the worker thread
    ...
    if result is not None:
        ...
        iface.messageBar().pushMessage('Sorry, something went wrong! See the message log for more information.', level=Qgs.CRITICAL)
...
def worker_error(e, exception_string, iface):
    # notify the user that something went wrong
    iface.messageBar().pushMessage('Something went wrong! See the message log for more information.', level=Qgs.CRITICAL, duration=3)
    QgsMessageLog.logMessage('Worker thread raised an exception: %s' % exception_string, 'SVIR worker', level=Qgs.CRITICAL)


def set_worker_message(message, progressMessageBar):
    progressMessageBar.setText(message)
...
class polygondivider:
    ...
    def run(self):
        ...
        * Run method that performs all the real work
        ...
        layers = QgsProject.instance().mapLayers().values() #qgis.core.QgsProject.instance().layerTreeRoot().layerOrder()
        layer_list = []
        for layer in layers:
